...
![WatchMyLAN Lite desktop dashboard](docs/images/dashboard-desktop.png)
...
- Network health panel, last-scan status, pauseable auto refresh, and bulk confirmation for known devices.
- Persistent sorting, persistent filters, search, attention filter, favorites and full or filtered CSV export.
...
- Automatic SQLite backups with configurable retention, in-app downloads and cleanup of old offline devices.
- Remote agents for VLANs or separate broadcast domains.
- Single-page frontend with no JavaScript build step.

## Responsive Interface

Desktop uses a dense comparison table. Phones and tablets use touch-friendly cards with IP, MAC, latency, connected time and primary actions without horizontal scrolling.

<img src="docs/images/dashboard-mobile.png" alt="WatchMyLAN Lite mobile dashboard" width="390">

> Screenshots use a fictional demonstration network and contain no real device data.

## Architecture

```mermaid
flowchart LR
    LAN["Local network"] --> SCAN["Scapy: ARP + ICMP"]
    SCAN --> API["FastAPI"]
    AGENT["VLAN agents"] --> API
    API --> DB["SQLite"]
    API --> UI["Web dashboard"]
    API --> ALERTS["Telegram / email / webhook"]
...
See [INSTALL.en.md](INSTALL.en.md) for the complete English installation, update, backup, agent and troubleshooting guide. The [Spanish guide](INSTALL.md) is the primary version.
...
|-- INSTALL.md              full installation guide in Spanish
|-- README.en.md            English documentation
`-- INSTALL.en.md           full installation guide in English
```
